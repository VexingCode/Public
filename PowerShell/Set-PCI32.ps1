<#
.SYNOPSIS
    Validate and check the protocols and ciphers for PCI 3.2 compliance.
.DESCRIPTION
    Checks the specified protocol for PCI 3.2 compliance. Intended for use with ConfigMgr on a per
    protocol/cipher basis for Configuration Item reporting and remediation.
.INPUTS
    -Protocol
...
Function Set-PCI32 {
    ...
    If ($Protocol) {
        ...
        If (!(Get-ItemProperty -Path $protocolPath -Name $ProtocolProperty -ErrorAction SilentlyContinue)) {
            ...
            Else {
                # It does not exist, and remediation not requested; report $false
                $false
            }
        }
        Else {
            ...
            If (!((Get-ItemPropertyValue -Path $protocolPath -Name $ProtocolProperty -ErrorAction SilentlyContinue) -eq $Value)) {
                ...
                Else {
                    # It is not the same, and remediation not requested; report $false
                    $false
                }
            }
            Else {
